...
def test_g251_an_allow_list_is_not_passed_by_leaving_the_counterparty_out(tmp_path):
    """A payment naming no payee is denied by the allow-list; a call moving no money is not, and a named stranger is denied either way."""
    dummy = DummyUpstream()
    dummy.start()
    try:
...
def test_g252_a_tool_call_without_its_counterparty_is_not_signed(tmp_path):
    """A tool call whose counterparty argument is absent, empty, not a string or oversized is unmapped, never forwarded."""
